      ...
      <PageShell>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-7 gap-3">
          {items.map((f) => {
            const s = flatSummary[f.id];
            return (
              <Link key={f.id} to="/society/explorer" search={{ flat: f.id, block: undefined as string | undefined }}>
      ...
      {blocks.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
          {blocks.map((b) => {
            ...
            return (
              <Link key={b.id} to="/society/explorer" search={{ block: b.id, flat: undefined as string | undefined }}>
                <Card className="rounded-2xl active:scale-[0.97] transition-transform">
                  <CardContent className="p-4">
