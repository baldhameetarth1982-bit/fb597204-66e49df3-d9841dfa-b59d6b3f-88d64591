...
export const generateCommunityDigest = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // If nothing this week, pull the last 30 days so a quiet week still yields a digest.
    let notices: { id: string; body: string; created_at: string }[] | null = null;
    if (!havePosts) {
